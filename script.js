(function () {
  ...
  function renderList(filter = "") {
    ...
    const all = loadAll().filter((p) =>
      (p.patient + p.doctor).toLowerCase().includes(filter.toLowerCase())
    );

    if (all.length === 0) {
      list.innerHTML = "<p class='muted'>No prescriptions</p>";
      return;
    }
    ...
    all.forEach((p) => {
      ...
      `;
      div.querySelector(".load").onclick = () => populateForm(p);
